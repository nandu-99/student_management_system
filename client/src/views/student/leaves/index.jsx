...
import { MdInsertChart, MdHistory, MdEventAvailable } from 'react-icons/md';
import { getRecentLeaveHistory, getUpcomingEvents } from 'api/api';
    ...
    const fetchData = async () => {
      ...
      const eventData = events.map((eve) => ({
        ...
        date:
          ...
          new Date(eve.endDate).toLocaleDateString()
            ? `${new Date(eve.date).toLocaleDateString()} - ${new Date(
                eve.endDate,
              ).toLocaleDateString()}`
            : `${new Date(eve.date).toLocaleDateString()}`,
      }));
